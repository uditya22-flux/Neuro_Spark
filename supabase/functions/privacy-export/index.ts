...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    let engine2: Record<string, unknown[]> = {
      layer1Sessions: [], stage2Handoffs: [], sublayerTelemetry: [], progression: [],
      taskExecutions: [], supportLadder: [], consistencyWindows: [], deepeningProfiles: [],
    };

    if (resolvedChildIds.length > 0) {
      const [sessRes, intakeRes, sensoryRes, layer1Res, handoffRes, telemetryRes, progressionRes, executionRes, supportRes, consistencyRes, profileRes] = await Promise.all([
        ...
        db.from('layer1_sessions').select('id, child_id, status, created_at, completed_at, expires_at').in('child_id', resolvedChildIds),
        db.from('stage2_handoffs').select('id, session_id, child_id, vertical_id, isolation_score, accuracy, recovery, engagement, speed, telemetry_reference, created_at').in('child_id', resolvedChildIds),
        db.from('sublayer_telemetry').select('id, session_id, child_id, task_id, vertical_id, source_type, accuracy, latency_ms, recovery, engagement, speed, source_confidence, isolation_score, telemetry_reference, expires_at, created_at').in('child_id', resolvedChildIds),
        db.from('layer_progression_state').select('id, session_id, child_id, vertical_id, current_layer, path_type, path_layers, completed_layers, status, support_level, updated_at').in('child_id', resolvedChildIds),
        db.from('layer_task_execution').select('id, task_id, session_id, child_id, vertical_id, layer_number, source_type, modality, support_level, accuracy, latency_ms, recovery, engagement, retry_count, hint_usage, answer_changes, skipped, metric_values, expires_at, created_at').in('child_id', resolvedChildIds),
        db.from('support_ladder_log').select('id, session_id, task_id, child_id, vertical_id, support_level, trigger_reason, outcome, created_at').in('child_id', resolvedChildIds),
        db.from('consistency_window').select('id, session_id, child_id, vertical_id, window_index, accuracy_stability_score, fatigue_score, created_at').in('child_id', resolvedChildIds),
        db.from('deepening_profiles').select('id, session_id, child_id, vertical_id, profile, telemetry_reference, created_at').in('child_id', resolvedChildIds),
      ]);
      sessions = sessRes.data ?? [];
      intakes = intakeRes.data ?? [];
      sensoryConfigs = sensoryRes.data ?? [];
      engine2 = {
        layer1Sessions: layer1Res.data ?? [], stage2Handoffs: handoffRes.data ?? [], sublayerTelemetry: telemetryRes.data ?? [],
        progression: progressionRes.data ?? [], taskExecutions: executionRes.data ?? [], supportLadder: supportRes.data ?? [],
        consistencyWindows: consistencyRes.data ?? [], deepeningProfiles: profileRes.data ?? [],
      };
    }
