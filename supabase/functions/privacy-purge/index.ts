...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const purgeError = await performCascadeDelete(
      serviceClient,
      guardianId,
      childId,
      purge.id,
    ...
    if (purgeError) {
      ...
      return internalError(
        "Purge could not be completed. The request has been logged.",
      );
    }
...
async function performCascadeDelete(
  ...
  try {
    if (childId) {
      ...
      await svc.from("layer_progression_state").delete().eq(
        "child_id",
        childId,
      );
      ...
      await svc.from("children").delete().eq("id", childId).eq(
        "guardian_id",
        guardianId,
      );
    } else {
      ...
      if (childIds.length > 0) {
        ...
        await svc.from("layer_task_execution").delete().in(
          "child_id",
          childIds,
        );
        await svc.from("layer_progression_state").delete().in(
          "child_id",
          childIds,
        );
        ...
        await svc.from("sensory_configurations").delete().in(
          "child_id",
          childIds,
        );
        await svc.from("child_experience").delete().in("child_id", childIds);
        await svc.from("adult_exploratory_note").delete().in(
          "child_id",
          childIds,
        );
